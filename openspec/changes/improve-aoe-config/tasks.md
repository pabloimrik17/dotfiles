...
- [x] 1.4 Confirm the gh-dash PR template variables used render correctly (`{{.PrNumber}}`, `{{.RepoName}}`, `{{.RepoPath}}` proven; verify `{{.Title}}`) and decide group depth (`reviews/{{.RepoName}}` → `reviews/owner/repo`). — `{{.RepoName}}` already renders as `owner/repo` in existing `b`/`t` bindings, so `reviews/{{.RepoName}}` = `reviews/owner/repo` (depth accepted). `{{.Title}}` live-render confirmation folded into 8.4.
...
- [x] 8.1 Add `f` (Queue in AoE) to the `prs:` keybindings: `wt -C {{.RepoPath}} switch pr:{{.PrNumber}} -x 'aoe add . -t "{{.Title}}"'` (no `-l`, no `--trust-hooks`). — Added (plain scalar; YAML valid).
