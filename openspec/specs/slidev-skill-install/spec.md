# Capability: slidev-skill-install

## Purpose

Provision the Slidev agent skill (`slidevjs/slidev`) globally via `skills.sh` during chezmoi setup, so Claude Code picks up Slidev's markdown syntax, magic-move code highlighting, layouts, transitions, diagrams, and export workflow on every fresh machine. Install shares the existing agent-skills group's confirmation prompt, skills-list cache, and error counter, and is scoped to Claude Code in this change (OpenCode is tracked by DOT-3).
