...
### D4 — Finder: builtin walk-up, then a repo-anchored walk-down; `.nvmrc` only
Walk up with shell parameter expansion (`${d%/*}`), not `$(dirname)` per level (the fork form cost +29 ms vs +1.6 ms). Up-walk covers root + ancestors. If nothing above, search down for the single `.nvmrc` — the confirmed real case (a polyglot monorepo whose Node app lives in a subdir like `apps/web/`, so the lone `.nvmrc` isn't at the repo root). The down-search is **anchored at the repository root** (`git rev-parse --show-toplevel`), bounded depth, skipping `node_modules/`/`.git/`. Anchoring at the repo root — not at `$PWD` — is what keeps the search scoped to *this* project: launching from a dir that merely holds sibling checkouts (e.g. `~/WebstormProjects`) is not a repo, so the probe fails fast and the shim passthroughs instead of scanning every sibling and activating an unrelated project's Node. **On 0 or >1 matches the shim passthroughs** (warning on >1) rather than guessing — the supported invariant is exactly one `.nvmrc` per repo. **Alternatives rejected:** (a) walk-up only — would never find a subdir `.nvmrc`, failing the stated requirement; (b) walk-down anchored at `$PWD` with shallowest-wins — scans sibling projects and silently activates (even cold-installs) an arbitrary unrelated Node when launched above a tree of repos.
...
- **[Trade-off] Multiple `.nvmrc` below the repo root** is treated as ambiguous: the shim warns and passthroughs to ambient Node rather than guessing. Acceptable for the confirmed single-subdir case (one `.nvmrc` per repo); a repo that genuinely pins per-subpackage gets ambient Node + a warning, not a silent wrong pick.
- **[Trade-off] In-repo launches with no `.nvmrc`** pay a `git rev-parse` + one bounded `find` within the repo on every launch (to catch the subdir case). Bounded by the repo and pruned (`node_modules`/`.git`), so single-to-low-double-digit ms for normal repos; the previous unbounded `find $PWD` that scanned sibling trees is gone.
