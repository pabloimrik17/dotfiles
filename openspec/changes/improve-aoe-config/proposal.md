...
- A chezmoi management strategy for `~/.agent-of-empires/config.toml` (likely a `modify_` script) that merges only the deliberately-managed keys and preserves AoE's runtime writeback (`[app_state]`/`[web]`/`[cockpit]`/`[logging]` and default-expanded tables), eliminating the recurring `chezmoi diff` thrash (live file is ~110 lines vs the 34-line source) and preventing `apply` from clobbering live state.
