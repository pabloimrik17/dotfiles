...
Builds on `agent-management-strategy`, which adopted Agent of Empires (`aoe` 1.9.5) and shipped a deliberately minimal `private_dot_agent-of-empires/config.toml` (34 lines). Stack: macOS (Apple Silicon primary), ghostty (truecolor, `clipboard-read = ask`, `copy-on-select = clipboard`), tmux with a hand-owned `~/.tmux.conf` (Catppuccin Mocha, `mouse on`, no `set-clipboard`/`allow-passthrough`), worktrunk (`wt` 0.55.0) for worktrees, Claude Code + OpenCode as agents, chezmoi-managed dotfiles. Claude Code agent teams are enabled (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in `settings.json`).
