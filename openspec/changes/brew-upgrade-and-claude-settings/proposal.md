...
- **Follow-up — stranded source paths**: renaming `dot_claude/settings.json.tmpl` to `modify_settings.json.tmpl` strands **18 requirements that still name the old path**, in capabilities this change does not otherwise touch: `claude-user-preferences` (8), `claude-code-plugins` (7), `claude-hud-config:74`, `mcp-global-config:29,:45`, `worktrunk-claude-plugin:15`. All 18 are semantically satisfied — only the filename is stale — so they are deliberately **not** corrected here. A separate change, `fix-stranded-spec-source-paths`, authored on this same branch, repoints them. `claude-hooks`'s three references need no follow-up: two belong to requirements this change removes, the third to the one it restates.
- **Post-merge apply**: a full `chezmoi apply` is still outstanding. Only `~/.config/bd/config.yaml` was applied here (targeted, to verify 7.13); `~/.config/worktrunk/config.toml` was also observed live. `chezmoi status` lists 7 pending targets: `.claude/settings.json`, `.config/lazygit/config.yml`, `.config/opencode/opencode.jsonc`, `.config/worktrunk/config.toml`, `.gitignore_global`, `.zshrc`, and the renamed `install-packages.sh`. `~/.local/share/chezmoi` is stale at `9d0bfc3` and must be brought current first, or the apply uses the pre-rename source; `.chezmoi.toml.tmpl` also differs between this branch and the source dir, so `chezmoi diff` warns "config file template has changed" and a `chezmoi init` is needed before the first full apply is trustworthy. The apply re-runs `run_onchange_install-packages.sh.tmpl` (brew installs, gh extensions, Claude marketplace registration) — expected, and the reason it was not run piecemeal here.
