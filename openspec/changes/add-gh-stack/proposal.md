...
- **Dependencies**: `gh` CLI ≥ 2.0 (installed: 2.96.0). No Homebrew formula exists; `gh extension install` is the only channel. The extension ships precompiled binaries, so no Go toolchain is needed
