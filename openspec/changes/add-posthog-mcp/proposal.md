...
- `claude-user-preferences`: adds PostHog to the MCP write tools kept out of `permissions.allow`, and corrects the stale "SHALL remain at the default ask level" guarantee — under `defaultMode: auto` those tools reach the safety classifier, not a user prompt (design D4). Descriptive only; no rule changes.
