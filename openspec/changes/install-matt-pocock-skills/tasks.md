...
- [ ] 5.1 **Requires an interactive TTY; only a human in a real terminal can complete this.** Run `chezmoi apply --source "$PWD"` twice against the render produced after `6afe6ba`/`7f2efb2` (last apply recorded script hash `808ad147f04d…`, current render is `93076ca72fed…`, so the `run_onchange` is still pending) and verify the second run reports the Matt plugin as already installed — Group 8 pre-scans with `grep`, so that holds on every host. Where `jq` is available, also verify all 24 selected standalone skill names are reported as already installed and nothing is reinstalled. Where `jq` is absent, verify the documented opposite: all 25 agent-scoped Group 9 entries (the 24 plus `gluestack-ui-v5`) reinstall, per the no-jq decision in `design.md`. Never run it from a TTY-less agent: `confirm()` uses `read -r reply </dev/tty` and, with no TTY, warns "No TTY available … defaulting to no" and returns 1 — every install group is skipped while chezmoi still records the script as executed in `scriptState`, falsely marking the `run_onchange` done.
