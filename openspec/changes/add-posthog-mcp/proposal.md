...
- **Permissions**: no `mcp__posthog__*` entry in `permissions.allow`. But with `defaultMode: "auto"`, staying off the allow list only means the classifier reviews the call, not the user — and no permission rule can do better here, because the MCP exposes every tool through one `exec` tool and Claude Code rules match tool names, never arguments. So writes are gated by a new `PreToolUse` hook, `dot_local/bin/executable_posthog-mcp-gate`, which reads the `command` argument and forces a prompt on anything that is not a recognised read. Fail-closed: unknown verbs, unknown tool names and unparseable input all prompt. This also mitigates the prompt-injection risk PostHog documents for analytics data entering agent context.
