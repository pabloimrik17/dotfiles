...
- The plugin bundles its own http MCP server (`https://mcp.posthog.com/mcp`, header `x-posthog-mcp-consumer: plugin`), surfacing as `plugin:posthog:posthog`, plus the `/posthog:flags`, `/posthog:insights`, `/posthog:errors` and `/posthog:experiments` slash commands.

**Explicitly not added to `MCP_HTTP_SERVERS`.** Registering `posthog` user-scope *on top of* the plugin would load the same PostHog tool set twice in every Claude Code session (`mcp__posthog__*` **and** `mcp__plugin_posthog_posthog__*`), burn context for zero extra capability, and require two separate OAuth logins against the same account. The exclusion is written as a `SHALL NOT` requirement so a later change does not "fix" it back in.
...
- **Known trade-off**: the Claude Code and OpenCode registrations are asymmetric (plugin vs. remote entry) instead of the usual "one array, both clients" pattern. Accepted — it is the only way to get the slash commands without duplicating the tool set.
