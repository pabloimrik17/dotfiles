...
- [x] 2.1 Add agent skills manual instructions to the non-macOS section listing all ten individual `npx -y skills add <repo> --skill <name> -g -y` commands

## 3. Verification

- [x] 3.1 Run the new group on the current machine and confirm `~/.claude/settings.json` is unchanged
- [x] 3.2 Confirm exactly the 10 expected skills are present in `~/.claude/skills/` and `~/.agents/skills/` after running
- [x] 3.3 Re-run the group and confirm already-installed skills are skipped
