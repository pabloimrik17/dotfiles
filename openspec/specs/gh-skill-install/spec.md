...
Detection SHALL NOT filter by `--agent`, even though the install side needs it to pick the destination. gh puts the skill body in `~/.agents/skills/<name>` and leaves `~/.claude/skills/<name>` as a symlink to it. `gh skill list` reports the resolved path, so these skills come back under the agent hosts `cline, universal, warp`, never `claude-code`. An `--agent claude-code`-filtered check therefore never matched: the script reinstalled on every run and `gh skill install` stopped at its "already exists, overwrite?" prompt, which died on the terminal's OSC 11 reply. That error exited non-zero, so chezmoi never recorded the `run_onchange` state and `chezmoi diff` never came back clean.
...
- **WHEN** a skill is symlinked from `~/.claude/skills` but reported under a non-Claude agent host
