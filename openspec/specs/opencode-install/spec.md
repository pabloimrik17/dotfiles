# Capability: opencode-install

## Purpose

Installation of opencode via its official installer (instead of Homebrew) so opencode's built-in auto-update stays active, including migration off a prior Homebrew install, idempotent re-runs, and in-script PATH availability for later install groups.
