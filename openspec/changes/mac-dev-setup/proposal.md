...
- `macos-defaults`: Chezmoi `run_once` script that applies macOS system preferences (Finder, Dock, trackpad, keyboard, hot corners, screenshots, Siri) idempotently. macOS-only via chezmoi template guard.
...
- **Files created**: `run_once_macos-defaults.sh.tmpl`
- **Files modified**: `run_once_install-packages.sh.tmpl` (new groups added, BREW_PACKAGES expanded, group numbering shifted)
- **Dependencies**: `mas` (brew formula) required for App Store group; `nvm` required for Node group
