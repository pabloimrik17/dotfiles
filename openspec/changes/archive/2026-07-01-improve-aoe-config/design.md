...
- **Prompt passing — REVISED post-merge (D4′).** The original plan (`wt … -x 'aoe add . … --extra-args' -- "<long inline prompt>"`) is WRONG: although worktrunk POSIX-escapes the post-`--` arg into a single `--extra-args` value, `aoe add` then **shell-splits `--extra-args`** (it's built for flags like `--port 8080`), so a multi-word prompt truncates to its first word (observed: "Spin"). Fix: put the review instructions in a versioned slash command `dot_claude/commands/review-team.md` and pass `--extra-args "/review-team {{.RepoName}}#{{.PrNumber}}"` — a single token (the command name) survives any split, and it's the same pattern as the working `b` binding (`claude /code-review:code-review …`). The command will migrate to the `experiments` plugin (`/experiments:review-team`) — MON-203.
- **`F` auto-launches — REVISED post-merge (D4″).** `F` (review team) was changed from queue-only to `aoe add -l`: a *review* is fire-and-forget, so it should start immediately rather than wait to be opened. Verified safe in `add.rs`: `if args.launch` calls `instance.start_with_size(...)` — a **background start that returns** (no `tmux attach`/terminal takeover), so under `wt -x` control still returns to gh-dash. `f` (queue a PR to work on later) stays queue-only (no `-l`).
