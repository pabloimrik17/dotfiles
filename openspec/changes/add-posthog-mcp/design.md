...
The residual exposure is accepted, matching how `mcp__memory__create_entities`, playwright and chrome-devtools already behave. PostHog is not singled out for stricter treatment: it is one more write-capable MCP behind the same classifier, and tightening the class is a broader change than this one.
