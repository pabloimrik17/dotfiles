...
- **D5 — gh skill idempotency via `--json`**: detection uses `gh skill list --agent claude-code --scope user --json skillName --jq '.[].skillName' | grep -qx "gh"` rather than parsing human output (`--scope user` because the default scans project scope too); the group lives inside the existing gh-extensions confirm gate, consistent with gh-dash/gh-enhance stanzas.
