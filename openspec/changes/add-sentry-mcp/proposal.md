...
- Enable the plugin by default in `dot_claude/modify_settings.json.tmpl`: `"sentry-mcp@sentry-mcp": true` in `enabledPlugins` and a `sentry-mcp` entry in `extraKnownMarketplaces` with `autoUpdate: true`
...
- **Files modified**:
    - `run_onchange_install-packages.sh.tmpl` — `CC_MARKETPLACES` (`:890-903`), `CC_PLUGINS` (`:905-934`), the OAuth line in "Manual Installation Required" (`:1225-1241`), and the "Claude Code plugins" section of the non-macOS fallback (`:1284-1288`)
    - `dot_claude/modify_settings.json.tmpl` — `enabledPlugins` (`:85-118`), `extraKnownMarketplaces` (`:122-216`), `permissions.allow` (`:315-388`) and a new `permissions.ask` array (`:389-392`)
    - `dot_config/opencode/opencode.jsonc` — the `mcp` block (`:18-34`)
...
- **No install-script counters change**: the group's `TOTAL_MCP` (8 stdio + 6 http) is untouched; `TOTAL_MP` goes 11 → 12 and `TOTAL_PL` goes 27 → 28
