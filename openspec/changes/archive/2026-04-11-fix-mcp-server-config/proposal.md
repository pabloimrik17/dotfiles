...
- **Existing MCP servers already in `~/.claude.json`**: No disruption — `claude mcp add --scope user` is idempotent (safe to run repeatedly; does not overwrite existing entries — reports "already exists in user config").
