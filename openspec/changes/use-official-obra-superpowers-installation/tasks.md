...
- [x] 1.1 Add `superpowers@git+https://github.com/obra/superpowers.git` to `dot_config/opencode/opencode.jsonc` immediately before the final websearch-cited plugin, then run `chezmoi cat -S . ~/.config/opencode/opencode.jsonc` and verify the resolved plugin array carries the exact package spec and ordering. `opencode debug config --pure` is not a source-level check: it merges the deployed config and auto-discovers `~/.config/opencode/plugins/`, so it reports entries that are in no source file.
