...
The zshrc SHALL define 7 pnpm aliases using the `p` prefix: `pi` (install), `pd` (dev), `pb` (build), `pt` (test), `pa` (add), `pr` (run), `px` (exec).
...
The zshrc SHALL define 7 bun aliases using the `b` prefix, symmetric with the pnpm `p` prefix: `bi` (install), `bd` (dev), `bb` (build), `bt` (test), `ba` (add), `br` (run), `bx` (bunx).
